...
public class EventsMDB implements MessageListener {
   ...
   */
  private BusinessEvent receiveBusinessEventMsg(BytesMessage rcvMessage)
   ...
   */
  private void sendResponse(CCCResponse cccResponse)
      throws NamingException, JMSException, JAXBException,
      PropertyException {
  ...
  private Event getEvent(BusinessEvent bEvent) {
    ...
    if (bEvent.getType().equals("reset")) {
      ...
      event = new Event(bEvent.getSequenceId(), bEvent.getOriginator(), bEvent.getResponder(),
          bEvent.getType(),
          bEvent.getStatus());
    }
    return event;
